...
const GroceryModal = ({ isOpen, onClose, onSave, onDelete, initialData, currentUser }) => {
    ...
    <ModalWrapper isOpen={isOpen} onClose={onClose} title={initialData ? "Edit Item" : "Add Grocery Item"}>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button onClick={handleSave} className="flex-1 bg-[var(--color-secondary)] text-white py-4 rounded-xl font-bold text-sm shadow-sm hover:bg-[var(--color-secondary-hover)] transition-colors">Save Item</button>
        </div>
      </div>
    </ModalWrapper>
  );
